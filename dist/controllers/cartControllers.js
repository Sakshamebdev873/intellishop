...
export const getCart = async (req, res) => {
    ...
    try {
        ...
        const items = await pool.query(`SELECT ci.id,ci.quantity,ci.added_at,
...
export const removeCartItem = async (req, res) => {
    ...
    if (!userId)
        return res.status(401).json({ erro: "Unauthorised" });
    try {
        const cartRes = await pool.query("SELECT id FROM carts WHERE user_id=$1", [
            userId,
        ]);
        if (cartRes.rows.length === 0)
            return res.status(404).json({ error: "Cart not found" });
        const cartId = cartRes.rows[0].id;
        const result = await pool.query("DELETE FROM cart_items WHERE cart_id=$1 AND product_id=$2 RETURNING id", [cartId, productId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: "Item not found in cart" });
        }
