...
export const getProductById = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await pool.query(`SELECT id,name,description,price,stock,category,created_at,updated_at FROM products where id=$1`, [id]);
...
export const updateProduct = async (req, res) => {
    ...
    try {
        ...
        values.push(id);
        const query = `UPDATE products SET ${fields.join(", ")}, updated_at=NOW() WHERE id=$${idx} RETURNING id, name, description, price, stock, category, created_at, updated_at`;
        const result = await pool.query(query, values);
        if (result.rows.length == 0) {
            return res.status(404).json({ error: "Product not found" });
        }
